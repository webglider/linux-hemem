				...
				unsigned long arg)
{
    ...
    if (((vma->vm_end - vma->vm_start) < (2 * 1024 * 1024)) || ((vma->vm_start % (2 * 1024 * 1024)) != 0)) {
      printk("fs/userfaultfd.c: userfaultfd_register: vma: 0x%lx - 0x%lx len: %ld\n", vma->vm_start, vma->vm_end, vma->vm_end - vma->vm_start);
    }
  skip:
		prev = vma;
		start = vma->vm_end;
			    ...
			    unsigned long arg)
{
	 ...
	 */
	ret = -EINVAL;
	printk("wei: in userfaultfd_copy, src: %llu, dst: %llu\n", uffdio_copy.src, uffdio_copy.dst);
			      ...
			      unsigned long arg)
{
	...
	printk("wei: cmd != UFFDIO_API, %d\n", cmd != UFFDIO_API);
	printk("wei: ctx->state == UFFD_STATE_WAIT_API, %d\n", ctx->state == UFFD_STATE_WAIT_API);
