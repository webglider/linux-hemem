...
static int inline bad_address(void *p)
{
        unsigned long dummy;
...
static int  inline page_walk(u64 address, u64* phy_addr)
{
	...
	//page_addr = pte_val(*pte) & PTE_PHYS_MASK;
        page_addr = pte_val(*pte) & PAGE_MASK;
...
//	u8 *dest;
	...
	page_walk(src, &dma_src);
	page_walk(dest, &dma_dest);
